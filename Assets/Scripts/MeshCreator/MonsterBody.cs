...
namespace MeshCreator
{
    ...
    public class MonsterBody : MonoBehaviour
    {
        ...
        private void UpdateBlendShapesFromCurve()
        {
            ...
            {
                float bonePosition = boneIndex / (float)(_bones.Count - 1);
        ...
        private void ApplyBoneWeights(List<Vector3> vertices, List<BoneWeight> boneWeights)
        {
            for (int i = 0; i < boneWeights.Count; i++)
            {
                ...
                var distanceAlongBone = vertices[i].z / (_boneSettings.Length * _bones.Count);
        ...
        private bool CurvesAreEqual(AnimationCurve curve1, AnimationCurve curve2)
        {
